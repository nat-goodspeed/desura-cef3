...
namespace tthread {
	...
	public:
		...
		template <class _mutexT>
		inline void wait(tthread::lock_guard<_mutexT> &aMutex)
		{
			wait(*aMutex.mMutex);
		}
		...
		template <class _mutexT>
		inline bool wait_timed(_mutexT &aMutex, unsigned int nSeconds)
		{
#if defined(_TTHREAD_WIN32_)
			// Increment number of waiters
			EnterCriticalSection(&mWaitersCountLock);
			...
			return pthread_cond_timedwait(&mHandle, &aMutex.mHandle, &ts) != ETIMEDOUT;
#endif
		}

		template <class _mutexT>
		inline bool wait_timed(tthread::lock_guard<_mutexT> &aMutex, unsigned int nSeconds)
		{
			return wait_timed(*aMutex.mMutex, nSeconds);
		}
	...
	private:
#if defined(_TTHREAD_WIN32_)
		bool _wait(DWORD timeout = INFINITE);
